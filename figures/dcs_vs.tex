...
\begin{axis}[heat,xticklabels={0,1,2,3,4,5,6}]
\addplot3[mat] table[z index=3] {data/TA-dcs2.dat}; \end{axis} \\ % RA (3)
};
% LABELS %
\node (n) at (0.15,-7.7) {\footnotesize Value of $n$};
\node[rotate=90] (k) at (-6.5,-0.1) {\footnotesize Value of $k$};
\node (tl) at (6.5,5.7)   {\small TL};
\node (dp) at (6.5,3.35)  {\small DP};
\node (cm) at (6.5,1.05)  {\small CM};
\node (at) at (6.5,-1.25) {\small AT};
\node (bw) at (6.5,-3.6)  {\small BW};
\node (ta) at (6.5,-5.9)  {\small TA};
\end{tikzpicture}
% COLOR BAR %
\begin{tikzpicture}
\begin{axis}[
