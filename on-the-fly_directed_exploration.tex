...
$\neg$\texttt{canBeWinningLoop($\SCC$)} entonces para alcanzar un estado marcado desde $e'$ debe salir de $\SCC$ a un estado $s \notin \SCC \cup \WES$ lo que implica $s \notin \WESS$ 
...
Si \texttt{canBeWinningLoop()} retorna false, entonces debemos chequear si $e' \in \LESS$.
...
Esto puede hacerse de forma más eficiente que con un punto fijo usando el Lemma~\ref{lem:findErrorsWorks} que muestra que alcanza con chequear si no es posible escapar de $\SCC$ alcanzando en un paso un estado que no esté en $\LES$. 
...
\texttt{findNewErrorsIn} no solo chequean si $e' \in \WESS$/$e' \in 
\LESS$ si no que también agregan estados ganadores/perdedores cuando pueden. La detección completa de nuevos estados ganadores y perdedores se hace finalmente con los procesos de propagación. 
...
Primero, notar que el algoritmo termina cuando logró determinar que $\init{e}$ está en $\LESS$ o $\WESS$. En el segundo caso, es simple construir un supervisor basándose en la estructura de exploración
$\structure$.\hfill$\qed$
